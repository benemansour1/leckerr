...
export default function Orders() {
 ...
 const activePhone = user?.phone || localStorage.getItem("phone");
  ...
  useEffect(() => {
    ...
    audioRef.current.volume = 1; // أعلى صوت
  }, []);
...
enabled: !!activePhone,
    refetchInterval: 30000, // كل 5 ثواني
  });
  ...
  const STATUS_MAP: Record<string, { label: string; color: string; icon: any }> = {
    ...
    preparing: { label: t.orders.status.preparing, color: 'text-amber-400 bg-amber-400/10', icon: Package },
    ready: { label: t.orders.status.ready, color: 'text-primary bg-primary/10', icon: Package },
  ...
  return (
    <CustomerLayout>
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-gold-gradient mb-8">{t.orders.title}</h1>
  ...
  <div className="mb-8 text-center space-y-4">
    ...
    <input
      className="border p-3 rounded-xl w-full max-w-sm mx-auto"
