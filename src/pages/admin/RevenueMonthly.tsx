...
export default function AdminRevenueMonthly() {
  ...
  const [month, setMonth] = useState(
    (currentObj.getMonth() + 1)
      .toString()
      .padStart(2, '0')
  );

  const [year, setYear] = useState(
    currentObj.getFullYear().toString()
  );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {language === 'he'
              ? 'ניתוח מכירות לחודש מסוים'
              : 'تحليل مبيعات شهر محدد'}
          </p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <select
            ...
            className="bg-input border-2 border-border rounded-xl px-4 py-2 outline-none"
          >
            {Array.from(
              { length: 12 },
              (_, i) => {
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6 h-[400px]">
            ...
            <h3 className="text-xl font-bold mb-6">
              {language === 'he'
                ? 'הימים הנמכרים ביותר'
                : 'الأيام الأعلى مبيعاً'}
            </h3>

            {revenue.breakdown.some(
              (b) => b.orders > 0
            ) ? (
              <ResponsiveContainer
                width="100%"
